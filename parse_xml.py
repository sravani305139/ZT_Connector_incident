...
def run_script(Number,Flow,Configuration_Item,Escalation_Group,Summary,Tower,userID,location,Department): # executes the use case flow in the backend process
    try:
        path = './Queue/InputFiles/'
        file = Number+'.xml'
        
        cmd = 'start /b powershell '+Flow+' '+Number+' '+Flow+' '+Configuration_Item+' '+"\""+Escalation_Group+"\""+' '+"\""+Summary+"\""+' '+userID+' '+location+' '+Department#+' >> D:\\custom_orch\\executing_files\\'+Number+'.txt' #command to choose the script flile
...
def exec_xml():
    try:
        ...
        pattern ='*.xml'
        fetch_data = ['Number','Flow','Configuration_Item','Escalation_Group','Summary','Tower','User_ID','user_location','user_department']
        ...
        try:
            ...
            for file in listOfFiles:
                if fnmatch.fnmatch(file,pattern): # matches the pattern 
                    ...
                    #print(dic)
                    run_script(str(dic[0]),str(dic[1]),str(dic[2]),str(dic[3]),str(dic[4]),str(dic[5]),str(dic[6]),str(dic[7]),str(dic[8]))
                    del dic[:]
